...
int paging_map_to(struct paging_4gb_chunk* directory, void* virt, void* phys, void* phy_end, int flags) {
  int res = 0;
  ...
  if ((uint32_t) phy_end % PAGING_PAGE_SIZE) {
    res = -EINVARG;
    goto out;
  }

  if ((uint32_t) phy_end < (uint32_t) phys) {
    res = -EINVARG;
    goto out;
  }

  uint32_t total_bytes = phy_end - phys;
